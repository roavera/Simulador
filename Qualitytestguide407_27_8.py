...
FT_TO_METERS = 0.3048
KTS_TO_METERS_PER_SEC = 1       #0.514444 knuts  Puede haber un error
...
class FlightController:
    ...
    def _initialize_pids(self):
        ...
        self.pid_yaw = PID(0.0015, 0.0001, 0.002, setpoint=0)  # Parámetros optimizados
        self.pid_yaw.output_limits = (-15, 15)
        
        # PID para distancia
        self.pid_distance = PID(2, 0.01, 0.1, setpoint=0)
    ...
    def _load_flight_path(self):
        ...
        try:
            ...
            logging.info(f"Cargyamados {len(points)} puntos de trayectoria")
            return points
    ...
    def calculate_yaw_control(self, heading_error, collective,airspeed):
        ...
        # Calcular salida del PID para el error de heading
        pid_output = self.pid_yaw(heading_error)
        
        # Efecto del collective (compensación de torque en hover)
        collective_effect = collective * 0.0  # 60% del collective como compensación
        
        # Factor de reducción basado en velocidad (0 a 60 nudos)
        speed_factor = max(0, 1 - (airspeed / 50))  # Lineal de 1 (0 nudos) a 0 (60 nudos)
        
        # Combinación de términos
        yaw_control = (pid_output + pid_output*collective_effect)*speed_factor
        
        # Limitar rango de salida
        return max(-15, min(15, yaw_control))
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                airspeed = dref_values[2][0] if dref_values[2][0] is not None else 0.0
                ...
                # Error de heading
                heading_error = self.calculate_heading_error(bearing - heading_correction*0, heading)
                ...
                if self.flight_phase == 'hover':
                    error_pos_z = 6111.0 - pos_z  # posición longitudinal deseada
                    error_pos_z = max(-10, min(10, error_pos_z))
                else:
                    error_pos_z = 0  # No considerar posición en otras fases
                
                self.pid_pitch.setpoint = error_speed * 0.7 - error_pos_z * 0.5
                cyclic_pitch = self.pid_pitch(pitch)
                cyclic_pitch = max(-0.7, min(0.7, cyclic_pitch))
                
                # Control de roll
                if self.flight_phase == 'hover':
                    g_heading = 0.0  # Ganancia para hover
                    error_pos_x = -38994 - pos_x  # Solo considerar posición en hover
                    error_pos_x = max(-10, min(10, error_pos_x))
                else:
                    g_heading = 0.2  # Ganancia para cruise/transición
                    error_pos_x = 0  # No considerar posición en otras fases
                
                error_lat = 0 - dref_values[3][0] if dref_values[3][0] is not None else 0.0
                error_lat = max(-2, min(2, error_lat))
                
                self.pid_roll.setpoint = (error_lat * 0.3 - error_pos_x * 0.2 + heading_error * g_heading)
